...
const CommunityEngagementForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!datePattern.test(formData.dateHired) || !datePattern.test(formData.startDate) || !datePattern.test(formData.endDate)) {
...
<input type="text" name="dateHired" value={formData.dateHired} onChange={handleChange} required />
</label>
</div>
<div  className='mb-4'>
<label>
...
<button className='bg-nucolor1 p-3 text-nucolor2' type="submit">Submit</button>
      {message && <p>{message}</p>}
</form>
</div>
  );
